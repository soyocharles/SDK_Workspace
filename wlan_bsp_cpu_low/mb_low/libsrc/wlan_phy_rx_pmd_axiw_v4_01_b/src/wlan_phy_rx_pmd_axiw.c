...
xc_status_t xc_wlan_phy_rx_pmd_axiw_getshmem(xc_iface_t *iface, const char *name, void **shmem)
{
    ...
    } else if (strcmp("CFO_EST_TIME_DOMAIN", name) == 0) {
        *shmem = (void *) & _config_table->CFO_EST_TIME_DOMAIN;
    ...
    } else if (strcmp("LTS_Corr_PeakType_Thresh", name) == 0) {
        *shmem = (void *) & _config_table->LTS_Corr_PeakType_Thresh;
    } else if (strcmp("CHAN_EST_SMOOTHING", name) == 0) {
        *shmem = (void *) & _config_table->CHAN_EST_SMOOTHING;
